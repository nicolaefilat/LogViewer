...
const ThemeToggler = () => {
	...
	useEffect(() => {
		...
		console.log("Changing refsun.current offset")
	}, [darkMode])
	
	
	return <button
		className="fixed top-5 right-5 w-16 h-8 flex justify-center items-center bg-gray-300 dark:bg-gray-700 border-slate-800 dark:border-black hover:border-blue-600 dark:hover:border-blue-300 border-[2px] rounded-2xl p-1"
		onClick={() => toggleDarkMode()}>
		<svg key={sunClass} id="sun" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"
			viewBox="0 0 24 24" className={`bg-white rounded-full p-[1px] m-[2px] -translate-x-1 ` + sunClass}>
			<path
		...
		<svg key={moonClass} xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"
			viewBox="0 0 24 24" className={`bg-black fill-white rounded-full p-[1px] m-[2px] rounded-full translate-x-1 ` + moonClass}>
			<path
